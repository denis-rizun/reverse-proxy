...
class LoadBalancer:
    def __init__(self, config: Config) -> None:
        ...
        self._upstream_cycle = cycle(self._upstreams)
        ...
        self._error_counters = {f"{up.host}:{up.port}": 0 for up in self._upstreams}
        self._circuit_open_until = {f"{up.host}:{up.port}": None for up in self._upstreams}
